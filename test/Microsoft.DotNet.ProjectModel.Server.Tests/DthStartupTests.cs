...
namespace Microsoft.DotNet.ProjectModel.Server.Tests
{
    public class DthStartupTests : IClassFixture<TestHelper>
    {
        ...
        {
            var projects = new string[] { "EmptyLibrary", "UnresolvedPackageSample", "EmptyConsoleApp" }
                .Select(name => _testHelper.FindSampleProject(name));

            // arrange
            using (var server = new DthTestServer(_testHelper.LoggerFactory))
            using (var client = new DthTestClient(server))
            {
                var contexts = projects.ToDictionary(proj => proj, proj => client.Initialize(proj));
